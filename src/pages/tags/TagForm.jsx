...
import { X, Save, Tag, Hash, Loader2 } from 'lucide-react';
...
/* ═══════════════════════════════════════════════════════════════
   MAIN EXPORT — rendered as overlay inside TagList
═══════════════════════════════════════════════════════════════ */
export default function TagForm({ editId, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    api.get(`/tags/${editId}/`)
      .then(({ data }) => {
        setForm({ name: data.name || '', type: data.type || 'general' });
        setFetching(false);
      })
      .catch(() => { toast.error('Failed to load tag'); onClose(); });
  }, [editId, isEdit]);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (isEdit) await api.put(`/tags/${editId}/`, payload);
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.28)', margin: '3px 0 0 11px' }}>
              {isEdit ? `Editing tag #${editId}` : 'Add a new content tag'}
            </p>
          </div>
          <button
