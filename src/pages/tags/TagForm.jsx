...
export default function TagForm() {
  ...
  useEffect(() => {
    ...
    api.get(`/tags/${id}/`).then(({ data }) => {
      ...
      setFetching(false);
    }).catch(() => { toast.error('Failed to load tag'); navigate('/tags'); });
  }, [id, isEdit, navigate]);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/tags')} className="p-2 hover:bg-gray-200 rounded-lg"><ArrowLeft size={20} /></button>
        <h1 className="text-2xl font-bold text-gray-800">{isEdit ? 'Edit Tag' : 'New Tag'}</h1>
      </div>
      <form onSubmit={handleSubmit} className="max-w-lg bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => navigate('/tags')}
