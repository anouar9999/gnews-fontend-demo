...
import {
  ...
  Meta,
  SectionHeader,
} from "../../components/public/landing/shared";
    ...
    >
      <div className="flex items-center gap-3 px-4 py-3 ">
        ...
        {search && (
          <button
            onClick={() => onSearch("")}
            className="text-[#444455] hover:text-white transition-colors"
          >
            <X size={13} />
          </button>
        )}
      ...
      >
        <img
          ...
          onError={(e) => {
            ...
            e.currentTarget.src = `https://picsum.photos/seed/${article.slug}/300/190`;
          }}
        />
      </div>
...
function PopularGamesWidget() {
  ...
  useEffect(() => {
    api
      .get("/games/", {
        params: { game_type: "popular", page_size: 6, ordering: "rank" },
      })
...
function AnticipatedGamesWidget() {
  ...
  useEffect(() => {
    api
      .get("/games/", {
        params: { game_type: "anticipated", page_size: 6, ordering: "rank" },
      })
      .then(({ data }) => setGames(data.results || data))
      .catch(() => {});
  }, []);
...
function Sidebar() {
  return (
    <aside className="flex flex-col gap-8">
      <LatestNewsWidget categorySlug="culture" />
      <TopArticlesWidget />
...
export default function CulturePage() {
  ...
  const fetchData = useCallback(
    (q = search) => {
      setLoading(true);
      api
        .get("/articles/", { params: buildParams(q) })
        .then(({ data }) => {
  ...
  const resultTitle = (() => {
    ...
    if (period === "today") return "Today's Culture News";
    if (period === "week") return "This Week in Culture";
    if (period === "month") return "This Month in Culture";
    return "Latest Culture News";
  })();

  return (
    <div className="text-white">
      <FontImport />
      <div className="max-w-[1280px] mx-auto px-22 sm:px-22 py-8">
        <SectionHeader title="Culture" color={ACCENT} />
