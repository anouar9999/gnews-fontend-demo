...
import {
  Gamepad2, ChevronDown, ChevronRight, Clock, Eye, TrendingUp, Flame, Search, X,
} from 'lucide-react';
...
function mapArticle(a) {
  return {
    ...
    tag:     a.category?.name || a.tags?.[0]?.name || 'GAMING',
    tagColor: ACCENT,
...
function FilterBar({ search, onSearch, sort, onSort, period, onPeriod, type, onType }) {
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(160deg,#0e0e18,#0a0a14)',
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 ">
        ...
        <input
          ...
          placeholder="Search gaming articles…"
      ...
      <div className="flex items-center gap-2.5 px-4 py-3 flex-wrap">
        ...
        <FilterDropdown options={TYPE_OPTIONS}   value={type}   onChange={onType}   color="#f59e0b" />
      </div>
    </div>
  );
}
...
function LatestNewsWidget({ categorySlug }) {
  ...
  useEffect(() => {
    ...
    api.get('/articles/', {
      params: { status: 'publie', category__slug: categorySlug, ordering: '-published_at', page_size: 7 },
    })
      .then(({ data }) => {
        setArticles((data.results || []).slice(0, 5).map(mapSidebarArticle));
      })
      .catch(() => {});
  }, [categorySlug]);
...
function PopularGamesWidget() {
  ...
  return (
    <div>
      <div
        style={{ padding: '10px 12px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <div style={{ width: 3, height: 16, borderRadius: 2, background: GREEN, flexShrink: 0 }} />
          <span style={{ fontSize: 11, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#fff' }}>
            Popular Games
          </span>
        </div>
        <Link
          to="/gaming"
          style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#444455', display: 'flex', alignItems: 'center', gap: 2 }}
        ...
        {games.map((g, i) => (
          ...
          >
            <div
              ...
              style={{ width: 38, height: 50, borderRadius: 3, background: '#111120' }}
            >
              {g.cover_image ? (
                <img
                  src={g.cover_image}
            ...
            <div className="min-w-0 flex-1">
              ...
              {(g.genre || g.players) && (
                <p style={{ fontSize: 10, color: '#555566', marginTop: 2 }}>{g.genre || g.players}</p>
              )}
            </div>
            {i < 3 && (
              <div
        ...
        {games.map(g => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              {g.release_display && (
                <p className="text-[10px] text-[#555566] mt-0.5">{g.release_display}</p>
              )}
            </div>
          </Link>
        ))}
...
export default function GamingPage() {
  ...
  const resultTitle = (() => {
    ...
    if (period === 'today')  return "Today's Gaming News";
    if (period === 'week')   return 'This Week in Gaming';
    if (period === 'month')  return 'This Month in Gaming';
  ...
  return (
    <div className="text-white">
      ...
      <div className="max-w-[1280px] mx-auto px-22  py-8">
        ...
        <SectionHeader title="Gaming news"  color={ACCENT} />
            ...
            <div className="flex items-end justify-between gap-4 mb-5">
              <div>
                ...
                {!loading && (
                  ...
                  <span className='text-orange'>{articles.length}{nextPage ? '+' : ''}</span>   article{articles.length !== 1 ? 's' : ''}
                  </p>
                )}
              </div>
              {hasActiveFilter && (
